...
% *rgb: The RGB array of 24 colors of MCC
% *srgb: The referenced RGB array of 24 colors of MCC
% *lab: The CIEL*A*B* of 24 colors of MCC
% *slab: The CIEL*A*B* of 24 colors of MCC
% *ref_label: The text label of the reference, will be the name of the ref
% in xls

function gen_mcc_report(out_file, rgb, srgb, lab, slab, ref_label)

    T1 = [3, 2, 17, 26];
    T2 = [3,30, 7, 54];
    
    xlswrite(out_file, {ref_label 'Sample' 'Mean-R' 'Mean-G' 'Mean-B' '' [ref_label,'-R'] [ref_label,'-G'] [ref_label,'-B'] '' 'Mean-L*' 'Mean-A*' 'Mean-B*' '' [ref_label,'-L*'] [ref_label,'-A*'] [ref_label,'-B*']}, getRange(T1(1)-2, T1(2), T1(3), T1(2)));
    xlswrite(out_file, rgb, getRange(T1(1), T1(2)+1, T1(1)+2, T1(4)));
    xlswrite(out_file, srgb, getRange(T1(1)+4, T1(2)+1, T1(1)+6, T1(4)));
    xlswrite(out_file, lab, getRange(T1(1)+8, T1(2)+1, T1(1)+10, T1(4)));
    xlswrite(out_file, slab, getRange(T1(1)+12, T1(2)+1, T1(1)+14, T1(4)));
    ...
    xlswrite(out_file, {'Delta-E' 'Delta-L' 'Delta-C' 'Delta-H' 'Delta-S'}, getRange(T2(1), T2(2), T2(3), T2(4)));
    xlswrite(out_file, [deltaE,deltaL,deltaC,deltaH,deltaS], getRange(T2(1), T2(2)+1, T2(3), T2(4)));
