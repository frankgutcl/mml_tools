...
function [left, right, comp_raw] = raw_pd_extract(raw_image, pd_pattern)
    ...
    left = zeros(pd.repeat.y*size(pd.pattern,1), pd.repeat.x*size(pd.pattern,2));
    right = zeros(pd.repeat.y*size(pd.pattern,1), pd.repeat.x*size(pd.pattern,2));
        ...
        for col=1:pd.repeat.x
            for i=1:size(pd.pattern, 1)
                for j=1:size(pd.pattern, 2)
                    ...
                    left((row-1)*size(pd.pattern,1)+i,(col-1)*size(pd.pattern,2)+j) = raw_image(y,x);
                    ...
                    right((row-1)*size(pd.pattern,1)+i, (col-1)*size(pd.pattern,2)+j) = raw_image(y, x);
